			...
			struct op *w, char *wp, daos_epoch_t we)
{
	...
	assert_int_equal(rc, expected_rrc);
...
conflicting_rw_exec(struct io_test_args *arg, int i, struct op *r, struct op *w)
{
	...
	for (k = 0; k < ARRAY_SIZE(emptyness); k++) {
		bool empty = emptyness[k];

		/* Read, then write. re > we. */
		re = mvcc_arg->epoch + 10;
		we = mvcc_arg->epoch;
		conflicting_rw_exec_one(arg, i, j, empty, r, rp, re, w, wp, we);
		j++;
		mvcc_arg->i++;
		mvcc_arg->epoch += 100;
